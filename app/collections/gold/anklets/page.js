...
export default function AnkletsPage() {
  ...
  // 🟢 Fetch gold anklets from backend
  useEffect(() => {
    async function fetchAnklets() {
      try {
        const res = await fetch(`/api/products/gold?category=anklet`);
        const data = await res.json();
        if (data.success) {
          setAnklets(data.products);
        } else {
          console.error("Failed to fetch anklets:", data.message);
        }
      } catch (err) {
        console.error("Error fetching anklets:", err);
      }
    }
    fetchAnklets();
  }, []);

  // 🟡 Filter by subcategory (Ladies / Gents) — case-insensitive
  const filteredAnklets =
    filter === "all"
      ? anklets
      : anklets.filter((item) => {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-yellow-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          ✨ Exquisite Gold Anklets Collection ✨
        </motion.h1>
        <motion.p
        ...
        >
          ...
          <strong>Ladies Anklets</strong> and{" "}
          <strong>Gents Anklets</strong> crafted with elegance and precision for all occasions.
        </motion.p>
      </div>
        ...
        {filteredAnklets.length > 0 ? (
          filteredAnklets.map((item) => (
            ...
            >
              ...
              <p className="text-gray-500 text-xs md:text-sm">{item.weight}g</p>
          ...
          <p className="text-center text-gray-600 col-span-full">
            No anklets found in database for the selected filter.
          </p>
        )}
      </div>
