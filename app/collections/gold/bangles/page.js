...
export default function BanglesPage() {
  ...
  useEffect(() => {
    const fetchBangles = async () => {
      try {
        // use category query param (server does case-insensitive partial match)
        const res = await fetch("/api/products/gold?category=bangle");
        if (!res.ok) throw new Error("Failed to fetch bangles");
        const data = await res.json();
        if (data.success) setBangles(data.products || []);
        else {
          console.error("Failed to fetch bangles:", data.message);
          setBangles([]);
        }
      } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-yellow-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          ✨ Exquisite Gold Bangles Collection ✨
        </motion.h1>
        <motion.p
        ...
        >
          Explore our exclusive range of <strong>Ladies Bangles</strong> and{" "}
          <strong>Gents Kadas</strong> crafted with precision and elegance for
          all occasions.
        </motion.p>
      </div>
      ...
      {/* Loading State */}
      {loading && (
        <p className="text-center text-yellow-700 font-medium">Loading bangles...</p>
      )}
          ...
          {filteredBangles.length > 0 ? (
            filteredBangles.map((item) => (
              ...
              >
                ...
                />
                <h3 className="text-sm md:text-base font-semibold text-yellow-800 mt-2 text-center">
                  {item.name}
                </h3>
                <p className="text-gray-600 text-xs md:text-sm">{item.purity}</p>
                <p className="text-gray-500 text-xs md:text-sm">{item.weight}</p>
                <p className="text-sm md:text-base font-bold text-yellow-700 mt-1">
                  ₹{item.price}
                </p>
                <button
                  onClick={() => {
            ...
            <p className="text-center text-gray-500 col-span-full">
              No bangles found for this category.
            </p>
          )}
        </div>
      )}

      {/* Popup removed: navigation now goes to product page on View Details */}
