...
export default function ChainsPage() {
  ...
  useEffect(() => {
    async function fetchChains() {
      try {
        const res = await fetch(`/api/products/gold?category=chain`);
        const data = await res.json();
        if (data.success) setChains(data.products || []);
        else setChains([]);
      } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-yellow-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          💫 Premium Gold Chains Collection 💫
        </motion.h1>
        <motion.p
        ...
        >
          Discover our curated selection of <strong>Men’s</strong> and{" "}
          <strong>Ladies Gold Chains</strong> designed with craftsmanship,
          purity, and timeless style.
        </motion.p>
      </div>
