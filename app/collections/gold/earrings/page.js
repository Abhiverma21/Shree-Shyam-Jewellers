...
export default function EarringsPage() {
  ...
  useEffect(() => {
    async function fetchEarrings() {
      try {
        const res = await fetch("/api/products/gold?category=earring");
        const data = await res.json();
        if (data.success) setEarrings(data.products || []);
        else setEarrings([]);
      } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-yellow-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          ✨ Exquisite Gold Earrings Collection ✨
        </motion.h1>
        <motion.p
        ...
        >
          ...
          our exclusive range of <strong>Ladies Earrings</strong> and{" "}
          <strong>Gents Studs & Hoops</strong> designed for elegance and confidence.
        </motion.p>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        {filteredEarrings.length > 0 ? (
          ...
          <p className="text-center text-gray-500 col-span-full">No earrings found for this category.</p>
        )}
      </div>

      {/* Popup modal removed: collection pages now navigate to product detail pages */}
