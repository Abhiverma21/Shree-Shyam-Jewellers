...
export default function PendantsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchPendants() {
      try {
        const res = await fetch("/api/products/gold?category=pendant");
        const data = await res.json();
        if (!mounted) return;
        if (data && data.success) setPendants(data.products || []);
        else setPendants([]);
      } catch (err) {
        console.error("Error fetching pendants:", err);
        if (mounted) setPendants([]);
      } finally {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-yellow-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          ✨ Exquisite Gold Pendants Collection ✨
        </motion.h1>
        <motion.p
        ...
        >
          Explore our exclusive range of <strong>Ladies Pendants</strong> and <strong>Gents Pendants</strong> crafted with elegance and precision for every occasion.
        </motion.p>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        {loading ? (
          <p className="col-span-full text-center text-gray-500">Loading pendants...</p>
          ...
          <p className="col-span-full text-center text-gray-500">No pendants found for this category.</p>
        )}
      </div>
