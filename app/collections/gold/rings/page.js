...
export default function RingsPage() {
  ...
  // 🟢 Fetch gold rings from backend
  useEffect(() => {
    async function fetchRings() {
      try {
        const res = await fetch(`/api/products/gold?category=ring`);
  ...
  // Accept either `subCategory` (camelCase) or `subcategory` (lowercase) coming from backend
  const filteredRings =
    filter === "all"
      ? rings
      : rings.filter((item) => {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-yellow-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          ✨ Exquisite Gold Rings Collection ✨
        </motion.h1>
        <motion.p
        ...
        >
          Discover timeless designs crafted with precision and love. Choose from
          our exclusive range of <strong>Ladies Rings</strong> and{" "}
          <strong>Gents Bands</strong> designed for elegance and confidence.
        </motion.p>
      </div>
