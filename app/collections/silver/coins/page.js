...
export default function CoinsPage() {
  ...
  // 🟢 Fetch silver coins from backend
  useEffect(() => {
    async function fetchCoins() {
      try {
        const res = await fetch(`/api/products/silver?category=coin`);
  ...
  // 🟡 Filter by category (Collector / Investment / Religious)
  const filteredCoins =
    filter === "all"
      ? coins
      : coins.filter((item) => {
          const category =
            (item.category || item.Category || "").toString().toLowerCase();
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-gray-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          Explore premium <strong>Silver Coins</strong> in 925 sterling silver —
          collectible, religious, and investment-grade designs.
        </motion.p>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        {filteredCoins.length > 0 ? (
          ...
          <p className="text-center text-gray-600 col-span-full">
            No coins found in database for the selected filter.
          </p>
        )}
      </div>
