...
export default function EarringsPage() {
  ...
  // 🟢 Fetch silver earrings from backend
  useEffect(() => {
    async function fetchEarrings() {
      try {
        const res = await fetch(`/api/products/silver?category=earring`);
        const data = await res.json();
        if (data.success) {
          setEarrings(data.products);
        } else {
          console.error("Failed to fetch earrings:", data.message);
        }
      } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-gray-50 to-white px-4 md:px-6 py-12 md:py-16 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 md:mb-12">
        ...
        >
          Explore our <strong>Ladies Earrings</strong> and <strong>Gents Earrings</strong> in pure 925 Silver — perfect for elegance and sophistication.
        </motion.p>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        {filteredEarrings.length > 0 ? (
          ...
          <p className="text-center text-gray-600 col-span-full">
            No earrings found in database for the selected filter.
          </p>
        )}
      </div>
